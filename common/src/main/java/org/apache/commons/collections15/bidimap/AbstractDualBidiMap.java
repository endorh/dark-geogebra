...
public abstract class AbstractDualBidiMap<K, V> implements BidiMap<K, V> {
	 ...
	 * <p>
	 * The two maps must be a matching pair, normal and reverse. They will
	 * typically both be empty.
	 * <p>
	 * Neither map is validated, so nulls may be passed in. If you choose to do
	 * this then the subclass constructor must populate the <code>maps[]</code>
	 ...
	 */
	protected abstract BidiMap<V, K> createBidiMap(Map<V, K> normalMap,
			Map<K, V> reverseMap, BidiMap<K, V> inverseMap);
	 ...
	 * <code>ResetableMapIterator</code>. This implementation relies on the
	 * entrySet iterator.
	 * <p>
	 * The setValue() methods only allow a new value to be set. If the value
	 * being set is already in the map, an IllegalArgumentException is thrown
	...
	// -----------------------------------------------------------------------
	/**
	 * Gets a keySet view of the map. Changes made on the view are reflected in
	...
	/**
	 * Gets a values view of the map. Changes made on the view are reflected in
	 * the map. The set supports remove and clear but not add.
	 *
	 ...
	 * value being set is already in the map, an IllegalArgumentException is
	 * thrown (as setValue cannot change the size of the map).
	 *
	...
	protected static class KeySet<K> extends View<K, Object, K>
		...
		protected KeySet(AbstractDualBidiMap<K, ?> parent) {
			super(parent.normalMap.keySet(),
					(AbstractDualBidiMap<K, Object>) parent);
		}
	...
	protected static class KeySetIterator<K>
		 ...
		 *            the parent map
		 */
		protected KeySetIterator(Iterator<K> iterator,
	...
	protected static class Values<V> extends View<Object, V, V>
		...
		protected Values(AbstractDualBidiMap<?, V> parent) {
			super(parent.normalMap.values(),
					(AbstractDualBidiMap<Object, V>) parent);
		}
	...
	protected static class ValuesIterator<V>
		 ...
		 */
		@SuppressWarnings("unchecked")
		protected ValuesIterator(Iterator<V> iterator,
	...
	protected static class EntrySetIterator<K, V>
		 ...
		 *            the parent map
		 */
		protected EntrySetIterator(Iterator<Map.Entry<K, V>> iterator,
