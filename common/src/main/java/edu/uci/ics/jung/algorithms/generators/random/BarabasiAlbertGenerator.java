...
public class BarabasiAlbertGenerator<V, E>
	...
	public BarabasiAlbertGenerator(Factory<Graph<V, E>> graphFactory,
			Factory<V> vertexFactory, Factory<E> edgeFactory, int init_vertices,
			int numEdgesToAttach, int seed, Set<V> seedVertices) {
	...
	public BarabasiAlbertGenerator(Factory<Graph<V, E>> graphFactory,
			Factory<V> vertexFactory, Factory<E> edgeFactory, int init_vertices,
			int numEdgesToAttach, Set<V> seedVertices) {
		this(graphFactory, vertexFactory, edgeFactory, init_vertices,
				numEdgesToAttach, (int) System.currentTimeMillis(),
				seedVertices);
	}
	...
	private void createRandomEdge(V newVertex,
		...
		do {
			...
			// already exists; note that because of the way edges are added, we
			// only need to check
			...
			// newVertex
			// (which has already been added to the graph, but not to
			// vertex_index)
