...
/**
 * An implementation of <code>Tree</code> in which each vertex has <= k
 * children. The value of 'k' is specified by the constructor parameter. A
 * specific child (edge) can be retrieved directly by specifying the index at
 * which the child is located. By default, new (child) vertices are added at the
 * lowest index available, if no index is specified.
 * 
 */
@SuppressWarnings("serial")
public class OrderedKAryTree<V, E> extends AbstractTypedGraph<V, E>
		implements Tree<V, E> {
	...
	/**
	 * Creates a new instance with the specified order (maximum number of
	 * children).
	 */
	...
	/**
	 * Returns an ordered list of {@code vertex}'s child vertices. If there is
	 * no child in position i, then the list will contain {@code null} in
	 * position i. If {@code vertex} has no children then the empty set will be
	 * returned.
	 * 
	 ...
	 * @return the depth of the vertex in this tree, or -1 if the vertex is not
	...
	public boolean addEdge(E e, V parent, V child, int index) {
		...
		if (!containsVertex(parent)) {
			throw new IllegalArgumentException(
					"Tree must already " + "include parent: " + parent);
		}
		if (containsVertex(child)) {
			throw new IllegalArgumentException(
					"Tree must not already " + "include child: " + child);
		}
		if (parent.equals(child)) {
			throw new IllegalArgumentException(
					"Input vertices must be distinct");
		}
		...
		// initialize VertexData for child; leave child's child_edges null for
		// now
	 ...
	 * @return 0 if <code>vertex</code> is the root, -1 if the vertex is not an
	 *         element of this tree, and 1 otherwise
	 */
	@Override
	...
	/**
	 * Returns true iff <code>v1</code> is the parent of <code>v2</code>. Note
	 * that if <code>v2</code> is the root and <code>v1</code> is
	 * <code>null</code>, this method returns <code>true</code>.
	 * 
	...
	public boolean addEdge(E edge, Collection<? extends V> vertices,
		...
		if (vertices.size() != 2) {
			throw new IllegalArgumentException(
					"'vertices' must contain " + "exactly 2 distinct vertices");
		}
		this.validateEdgeType(edge_type);
		Pair<V> endpoints;
	 ...
	 *         in this tree, or <code>null</code> if it has no child at that
	 *         position
