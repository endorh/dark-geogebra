...
public abstract class GeoGebraToPgf extends GeoGebraExport {
	...
	private void addText(String st0, boolean isLatex, int style) {
		...
		if (format == FORMAT_LATEX) {
			...
			// do nothing
			case 1 -> code.append("{\\bf ");
			case 2 -> code.append("{\\em ");
			case 3 -> code.append("{\\em \\bf");
			}
			code.append(st);
			switch (style) {
			default -> {
			...
			// do nothing
			case 1 -> code.append("\\bf{");
			case 2 -> code.append("\\it{ ");
			case 3 -> code.append("\\it{\\bf{");
			}
			code.append(st);
			switch (style) {
			default -> {
	...
	protected boolean fillSpline(GeoCurveCartesian[] curves) {
		...
		for (GeoCurveCartesian curve : curves) {
			...
			xprec = curve.getFunX().value(curve.getMinParameter());
			x = xprec;
		...
		fill.append("\\draw").append(liopco).append("(").append(x).append(",").append(y)
				.append(") circle(0pt);\n");
		code.append(fill);
		return true;
	}
